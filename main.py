...
def select_section():
    """Prompt the user to select a section by ID."""
    # Get a list of all sections
    sections = Session.query(Section).all()

    # Display all sections to the user
    for section in sections:
        print(f"ID: {section.section_id}, {section}")

    # Prompt the user to select a section by ID
    while True:
        try:
            selected_id = int(input("Enter the ID of the section you wish to select: "))
            selected_section = next((section for section in sections if section.section_id == selected_id), None)
...
def enroll_student_to_section(sess: Session):
    student: Student = select_student(sess)
...
def enroll_section_to_student(sess: Session):
    section: Section = select_section(sess)
    student: Student = select_student(sess)
...
def unenroll_student_from_section(sess: Session):
    student: Student = select_student(sess)
    section: Section = select_section(sess)

    if not student.is_enrolled_in(section):
        print(f"{student.name} is not enrolled in {section.name}.")
        return
...
def unenroll_section_from_student(sess: Session):
    section: Section = select_section(sess)
    student: Student = select_student(sess)

    if not section.has_student(student):
        print(f"{section.name} does not have {student.name} enrolled.")
        return
...
def list_student_enrollments(sess: Session):
    student: Student = select_student(sess)
    sections = student.get_enrolled_sections()
    for section in sections:
        print(f"Enrolled in: {section.name}")


def list_section_enrollments(sess: Session):
    section: Section = select_section(sess)
...
def delete_section(sess: Session):
    ...
    if section.has_enrollments():
        print("Cannot delete a section that has enrollments.")
        return
